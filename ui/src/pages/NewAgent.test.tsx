...
describe("NewAgent Claude subscription login", () => {
  ...
  it("shows Log in for a Claude sandbox before Create agent", async () => {
    ...
    await flushUntil(() => Boolean(findButton(result.container, "登录")));

    const loginButton = findButton(result.container, "登录");
  ...
  it("applies a stored token first and starts a replacement login with the captured version", async () => {
    ...
    await flushUntil(() => Boolean(findButton(result.container, "重新登录以替换")));
    await clickByText(result.container, "重新登录以替换");
